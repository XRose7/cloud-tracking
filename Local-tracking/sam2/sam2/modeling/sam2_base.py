...
class SAM2Base(torch.nn.Module):
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            for t_pos, prev in t_pos_and_prevs:
                if prev is None or prev["maskmem_features"] is None:
    ...
    def update_memory_from_grpc(self, memory_dict):
        self.memory_dict.update(memory_dict)
        
    
    
    def _get_memory_from_cloud(self, frame_idx):
        original_shape = self.memory_dict["original_shape"]
        maskmem_features = torch.tensor(self.memory_dict[frame_idx]["maskmem_features"], dtype=torch.float32).reshape(original_shape)
        
        # 假设你拿到的是 pos_enc_all，和每个 tensor 的 shape 一样
        flat_pos_enc_list = self.memory_dict[frame_idx]["maskmem_pos_enc"]
        num_tensors = len(flat_pos_enc_list) // np.prod(original_shape)

        maskmem_pos_enc = []
        for i in range(num_tensors):
            start = i * np.prod(original_shape)
            end = (i + 1) * np.prod(original_shape)
            array = np.array(flat_pos_enc_list[start:end], dtype=np.float32)
            tensor = torch.tensor(array).reshape(original_shape)
            maskmem_pos_enc.append(tensor)
    ...
    def _encode_memory_in_output(
        ...
        if self.num_maskmem > 0:
            maskmem_features, maskmem_pos_enc = self._get_memory_from_cloud(frame_idx)
